...
public class Car {
    ...
    private char mode; // 변속기 모드 (P, R, S, D)
    private boolean start;// 시동이 걸린 여부


    public Car(String model) {
        System.out.println("모델명은 " + model);
        this.model = model;
    }
    ...
    public void setSpeed(int speed) {
        ...
        if (mode == 'D'|| mode== 'R' ){
            if (mode == 'R' && speed > 40){
                System.out.println("후진은 40이상 가속할수없습니다.");
                return;
            }
    ...
    public void setMode(char mode) {
        switch (mode){
            case 'D' : case 'R' : case 'P' : case 'N' :
                ...
                break;
            default:
                System.out.println("변속기 모드를 확인하세요!");
        }
    }

    //Boolean 타입의 getter는 is+필드명으로 자동입력됨.
/*
...
/*    public void setStart(boolean start) {
    ...
    public void engineStop(){
        if (speed> 0){
            System.out.println("주행중에는 시동을 끝수없습니다.");
            return;
        }
        if (mode != 'P'){
